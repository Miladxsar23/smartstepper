...
const SmartStepper = <S extends string>({ config }: SmartStepperProps<S>) => {
    ...
    useForm({
      resolver: async (data) => {
        ...
        if (schema && typeof (schema as { safeParse?: unknown }).safeParse === 'function') {
          const result = (schema as { safeParse: (data: unknown) => { success: boolean; data?: FieldValues; error?: { errors: Array<{ path: (string | number)[]; message: string }> } } }).safeParse(data);
          if (result.success) {
            ...
            (result.error?.errors || []).forEach((e: { path: (string | number)[]; message: string }) => {
              if (e.path && e.path.length > 0) {
                errors[e.path.join('.')] = { type: 'manual', message: e.message };
              }
            });
  ...
  const currentStepSchemaFields = useMemo(() => {
    ...
    if (schema && typeof (schema as { fields?: unknown }).fields === 'object') {
      return Object.keys((schema as { fields: object }).fields ?? {}) as Path<FieldValues>[];
    }
    // Zod duck-typing (object schemas)
    if (schema && typeof (schema as { _def?: unknown })._def === 'object' && ((schema as unknown as { _def: { shape?: unknown } })._def.shape)) {
  ...
  const handleNextStep = async () => {
    ...
    if (!isValid) return;
    const next = config.orchesration[step]?.next?.(getValues());
    if (next) {
